          ...
          <div class="hero-content">
            ...
            <div class="hero-actions animate-fade-in-up animate-delay-2">
              ...
              <div class="share-wrapper">
                <button 
                  class="btn btn-icon btn-lg share-btn"
                  (click)="shareMenuOpen.set(!shareMenuOpen())"
...
export class CityComponent implements OnInit, OnDestroy {
  ...
  getShareOptions() {
    ...
    return {
      ...
      description: city.tagline || `Scopri ${city.name}, una destinazione incredibile in ${city.country}`,
      url: this.socialShare.getCityShareUrl(city.id),
      image: city.heroImage || city.thumbnailImage,
      hashtags: ['travel', 'viaggi', 'destinazioni', city.name.toLowerCase().replace(/\s+/g, '')]
    };
  }
  ...
  shareToFacebook(): void {
    ...
    if (options) {
      this.socialShare.shareToFacebook(options);
  ...
  shareToTwitter(): void {
    const options = this.getShareOptions();
    if (options) {
      this.socialShare.shareToTwitter(options);
      this.shareMenuOpen.set(false);
    }
  }
  ...
  shareToLinkedIn(): void {
    const options = this.getShareOptions();
    if (options) {
      this.socialShare.shareToLinkedIn(options);
      this.shareMenuOpen.set(false);
    }
  }
  ...
  shareToWhatsApp(): void {
    const options = this.getShareOptions();
    if (options) {
      this.socialShare.shareToWhatsApp(options);
      this.shareMenuOpen.set(false);
    }
  }
  ...
  async copyLink(): Promise<void> {
    const options = this.getShareOptions();
    if (options) {
      const success = await this.socialShare.copyToClipboard(options);
      if (success) {
